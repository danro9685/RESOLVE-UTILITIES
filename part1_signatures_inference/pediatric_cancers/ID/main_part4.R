...
# load the data
load("final_data/id_counts.RData")
counts = as.matrix(id_counts)
...
for(i in 1:length(signatures_ordering)) {
    ...
    curr_signature = rep(signatures_ordering[i],nrow(counts))
    ...
    for(j in rownames(counts)) {
        curr_cos = c(curr_cos,as.numeric(cosine(curr_predictions[j,],counts[j,])))
    }
    curr_unexplained_mutations = as.numeric(incremental_assignment$unexplained_mutations[rownames(counts)])
    curr_unexplained_mutations_norm = as.numeric(curr_unexplained_mutations/rowSums(counts))
...
for(i in unique(plot_data$SIGNATURES)) {
    ...
    LINE_PREDICTIONS = c(LINE_PREDICTIONS,(length(which(PREDICTIONS[which(SIGNATURE==i)]>0.95))/nrow(counts)))
...
COSMIC = (length(which(COSMIC_goodness_fit>0.95))/nrow(counts))
...
RANGE = LINE_SIGNATURE
COSMIC = as.numeric(COSMIC_assignment$unexplained_mutations[rownames(counts)])
COSMIC = mean(as.numeric(COSMIC/rowSums(counts)),na.rm=TRUE)
