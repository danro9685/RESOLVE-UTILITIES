...
# load the data
load("final_data/dbs_counts.RData")
dbs_counts = as.matrix(dbs_counts)
...
for(i in 1:length(signatures_ordering)) {
    ...
    curr_signature = rep(signatures_ordering[i],nrow(dbs_counts))
    curr_predictions = ((incremental_assignment$alpha%*%incremental_assignment$beta)+incremental_assignment$unexplained_mutations)
    curr_cos = NULL
    for(j in rownames(dbs_counts)) {
        curr_cos = c(curr_cos,as.numeric(cosine(curr_predictions[j,],dbs_counts[j,])))
    }
    curr_unexplained_mutations = as.numeric(incremental_assignment$unexplained_mutations[rownames(dbs_counts)])
    curr_unexplained_mutations_norm = as.numeric(curr_unexplained_mutations/rowSums(dbs_counts))
...
for(i in unique(plot_data$SIGNATURES)) {
    ...
    LINE_PREDICTIONS = c(LINE_PREDICTIONS,(length(which(PREDICTIONS[which(SIGNATURE==i)]>0.95))/nrow(dbs_counts)))
...
COSMIC = (length(which(COSMIC_goodness_fit>0.95))/nrow(dbs_counts))
reference = (length(which(reference_signatures_goodness_fit>0.95))/nrow(dbs_counts))
...
RANGE = LINE_SIGNATURE
COSMIC = as.numeric(COSMIC_assignment$unexplained_mutations[rownames(dbs_counts)])
COSMIC = mean(as.numeric(COSMIC/rowSums(dbs_counts)),na.rm=TRUE)
reference = as.numeric(reference_signatures_assignment$unexplained_mutations[rownames(dbs_counts)])
reference = mean(as.numeric(reference/rowSums(dbs_counts)),na.rm=TRUE)
plot_data = data.frame(VALUE=VALUE,RANGE=RANGE,check.names=FALSE,stringsAsFactors=FALSE)
dev.off()
dev.new()
